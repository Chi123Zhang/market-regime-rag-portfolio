        ...
        if run_button:
            ...
            if df.empty:
                ...
                if use_llm_explanation:
                    ...
                    st.json(llm_output)
                    st.caption(
                        "Important: the final regime prediction above comes from "
                        "feature-space balanced KNN. The LLM only summarizes retrieved "
                        "historical evidence and does not overwrite the prediction."
                    )
                else:
                    st.info(
